...
int main(){
	...
	while(choice!=0){
		switch(choice){
			case 1: printf("Enter the key : "); scanf("%d",&key); if(!findN(root,key,0)) printf("The key is not in the tree!\n"); break;
...
int insertN(terP *s,int a){
	...
	//if it is not empty find right place for key and insert
		...
		if(pow((*s)->data,2)<40 && (*s)->data!=1){
			if(a<=pow((*s)->data,2)) insertN(&((*s)->left),a);
			if(a>pow((*s)->data,2) && a<pow((*s)->data,3)) insertN(&((*s)->middle),a);
			if(a>=pow((*s)->data,3)) insertN(&((*s)->right),a);
		}
		else if((pow((*s)->data,2)>40 || (*s)->data==1) && (*s)->data<80){
			if(a<(*s)->data) insertN(&((*s)->left),a);
			if(a>(*s)->data) insertN(&((*s)->right),a);
		}
		else if((*s)->data>=80){
			if(a<=((*s)->data-10)) insertN(&((*s)->left),a);
			if(a>((*s)->data-10) && a<((*s)->data+10)) insertN(&((*s)->middle),a);
			if(a>=((*s)->data+10)) insertN(&((*s)->right),a);
...
terP findN(terP s,int a,int i){
	...
	if(pow(s->data,2)<40 && s->data!=1){
		...
		if(a>pow(s->data,2) && a<pow(s->data,3)){
			printf("middle\n");
			i++;
			findN(s->middle,a,i);
		}
